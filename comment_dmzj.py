...
cookieSet = [] 
proxiesIpSet = ['62.148.67.110:81','150.138.253.70:808','183.220.145.3:80','58.220.95.79:10000','58.220.95.80:9401','58.220.95.78:9401','150.138.253.72:808','194.152.235.171:8080','123.56.161.63:80','52.191.103.11:3128','1.255.48.197:8080',
]

def commentFunc(comId, context, interval, cookieNo, imagineNameList):
    headers = {'User-Agent': random.choice(agents)}
    cookie = cookieSet[cookieNo%len(cookieSet)]
    cookies = {'cookie': cookie}

    #评论添加图片
    imgStr = uploadImagine.getImagineStrByList(cookie, imagineNameList)

    data = {
        'callback':'add_sucess',
        'obj_id':comId,
        'sender_uid':111015965,
        'content':context,
        'to_uid':0,
        'to_comment_id':0,
        'sender_terminal':1,
    ...
    print("cookieNo=", cookies)
    #ipPort = random.choice(proxiesIpSet)
    # proxies = {
    #     "http": "http://" + ipPort,
    #     "https": "https://" + ipPort,
    # }
    ...
    resultObj = re.search(r'\d+', resultText)
    if result:
        print("comment success, id=", comId)
        time.sleep(interval)
    elif resultObj and resultObj.group(0) == '1': #账号被封
        cookieSet.remove(cookieSet[cookieNo%len(cookieSet)])
        if len(cookieSet) == 0:
            print("文件中的所有cookie已经全部被封，请更新")
            file.saveCookiesToFile('cookies.txt', cookieSet)
            os.system("pause")
        else: 
            #更新文件
            print("删除旧cookie:", cookies["cookie"])
            file.saveCookiesToFile('cookies.txt', cookieSet)
